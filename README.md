...
def generate_prime(bits):
    ...
    while True:
        # Gera número aleatório com bit mais alto e mais baixo ligados
        # para garantir tamanho e ímpar
        candidate = random.getrandbits(bits)
        candidate |= (1 << (bits - 1))  # garante tamanho em bits
        candidate |= 1                  # garante ímpar
...
def generate_keys(bits=512):
    ...
    # Se não for coprimo com phi, escolhe outro
    if gcd(e, phi) != 1:
        e = 3
        while gcd(e, phi) != 1:
            e += 2
...
def text_to_blocks(text, n):
    ...
    Estratégia:
    - Descobre o número máximo de bytes por bloco tal que:
        bloco_int < n
    - Divide os bytes da mensagem nesses blocos.
    - Cada bloco é convertido para inteiro.

    Retorna:
        blocks          -> lista de inteiros
        block_size      -> tamanho fixo do bloco em bytes
        original_length -> quantidade total de bytes originais
    """
    data = text.encode("utf-8")

    # Descobre tamanho máximo de bloco em bytes para caber em n
    block_size = 1
...
def blocks_to_text(blocks, block_size, original_length):
    ...
    for i, block in enumerate(blocks):
        ...
        # Para o último bloco, pode haver zeros à esquerda por causa do tamanho fixo.
        # Depois recortamos pelo tamanho original total.
